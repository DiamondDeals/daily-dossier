...
def generate_combined_markdown(results):
    """Generate combined markdown digest with freshness filtering.
    Items shown in prior dossiers (last 3 days) are deprioritized so
    every run surfaces genuinely new content."""
    from track_duplicates import load_seen, purge_old, pick_fresh_top, save_seen

    timestamp = datetime.now().strftime("%Y-%m-%d %I:%M %p PST")
    total = sum(r['count'] for r in results.values())

    # Load seen URLs and purge entries older than 3 days
    seen = load_seen()
    seen = purge_old(seen, days=3)
...
"""
    ...
    # Helper to get item list from a platform result
    def get_items(platform):
        r = results[platform]
        return r.get('posts', r.get('videos', r.get('articles', [])))

    # --- Reddit (top 10, fresh first) ---
    if results['reddit']['count'] > 0:
        top, seen = pick_fresh_top(get_items('reddit'), seen, 10)
    ...
    if results['twitter']['count'] > 0:
        top, seen = pick_fresh_top(get_items('twitter'), seen, 10)
        for i, post in enumerate(top, 1):
    ...
    if results['youtube']['count'] > 0:
        top, seen = pick_fresh_top(get_items('youtube'), seen, 10)
    ...
    if results['moltbook']['count'] > 0:
        top, seen = pick_fresh_top(get_items('moltbook'), seen, 10)
        for i, post in enumerate(top, 1):
            md += f"\n**{i}. {post.get('title', 'Untitled')}**\n"
    ...
    if results['health']['count'] > 0:
        top, seen = pick_fresh_top(get_items('health'), seen, 10)
        for i, post in enumerate(top, 1):
            md += f"\n**{i}. {post.get('title', 'Untitled')}**\n"
    ...
    if results['rss']['count'] > 0:
        top, seen = pick_fresh_top(get_items('rss'), seen, 15)
    ...
    save_seen(seen)
    fresh_tracked = len(seen)
    print(f"📋 Freshness: tracking {fresh_tracked} shown URLs (3-day window)")
